      ...
      <div className="results-grid">
        <div className="result-card fedavg-card">
          ...
          <div className="result-content">
            ...
            <p className="result-description">The weighted average across all client instances (power factor)</p>
          </div>
        </div>
            ...
            <div className="trust-scores-list">
              {trustScores
                ...
                .map((score) => (
                  <div
                    key={score.clientId}
                    className={`trust-score-item ${attackedNodes.includes(score.clientId) ? "attacked" : ""} ${
                      score.trustLevel === "Perfect" ? "perfect" : score.trustLevel === "High" ? "high" : "low"
                    }`}
                  >
                    ...
                    <div className={`trust-level-badge trust-level-${score.trustLevel.toLowerCase()}`}>{score.trustLevel}</div>
                  </div>
                ))}
            </div>
            <p className="result-description">Trust score (0-1 scale) and trust level for each client</p>
          </div>
        </div>
          ...
          <div className="result-content">
            {attackedNodes.length > 0 ? (
              <div className="attacked-nodes">
                <div className="warning-message">{attackedNodes.length} node(s) detected as compromised</div>
              ...
              <div className="safe-message">No attacked nodes detected. All clients are secure.</div>
            )}
            <p className="result-description">Detection of False Data Injection Attack (FDIA) nodes</p>
          </div>
        </div>
      </div>
    </div>
